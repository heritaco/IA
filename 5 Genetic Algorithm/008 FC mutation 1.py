...
def generate_genome(rutas: List[Ruta], start_point: str, end_point: str) -> Genome:
    ...
    # Ensure that indexes is a list of integers
...
def mutate_genome(genome: Genome, rutas: List[Ruta], start_point: str, end_point: str) -> Genome:
    ...
    # Check if genome[0] is a list of lists
    if isinstance(genome[0], list):
        # Flatten genome[0]
        genome_0_flattened = [item for sublist in genome[0]
                              for item in sublist]
    else:
        # If genome[0] is not a list of lists, just use it as is
        genome_0_flattened = genome[0]
    # Flatten genome[0]
    genome_0_flattened = [item for sublist in genome[0] for item in sublist]
    # Check if genome[0] has more than one element
    if len(genome_0_flattened) > 1:
        # Choose a random index to mutate
        index_to_mutate = random.randint(1, len(genome_0_flattened) - 1)
        new_start_point = rutas[genome_0_flattened[index_to_mutate - 1]].end
        # Generate a new route
        new_route = generate_genome(rutas, new_start_point, end_point)

        # Create the new genome
        new_genome = list(genome)
        # Convert to list before concatenating
        new_genome_0_flattened = [
            item for sublist in new_genome[0] for item in sublist]
        new_genome[0] = new_genome[0][:index_to_mutate] + \
            new_genome_0_flattened
        new_genome[1] = sum(rutas[i].time for i in new_genome_0_flattened)
        new_genome[2] = [rutas[i].linea for i in new_genome_0_flattened]
        new_genome[3] = [(rutas[i].start, rutas[i].end)
                         for i in new_genome_0_flattened]
        ...
        # If genome[0] has only one element, just return the original genome
        new_genome = genome
...
def mutate_best_genome(population: Population, rutas: List[Ruta], start_point: str, end_point: str) -> Genome:
    ...
    mutated_best_genome = mutate_genome(
        best_genome, rutas, start_point, end_point)
...
def create_new_population(population: Population, rutas: List[Ruta], elite_size: int, mutation_rate: float) -> Population:
    ...
    mutations = [mutate_genome(genome, rutas, start_point, end_point)
                 for genome in elite if random.random() < mutation_rate]
...
new_population = create_new_population(
    population, rutas, elite_size, mutation_rate)
...
def create_new_population(population: Population, rutas: List[Ruta], elite_size: int, mutation_rate: float) -> Population:
    ...
    elite, fitness_values, sorted_by_fitness = elitism(
        population, rutas, elite_size)
    random_genomes = [generate_genome(
        rutas, start_point, end_point) for _ in range(size_pop - len(elite))]
...
def create_new_population(population: Population, rutas: List[Ruta], elite_size: int, mutation_rate: float) -> Population:
    ...
    # Mutate the elite genomes to create the rest of the population
    mutated_genomes = [mutate_genome(genome, rutas, start_point, end_point) for _ in range(
        (size_pop - len(elite)) // len(elite)) for genome in elite]

    new_population = elite + mutated_genomes
    return new_population
    ...
    for _ in range(num_generations):
        ...
        for i in range(len(population)):
            if random.random() < mutation_rate:
                population[i] = mutate_genome(
                    population[i], rutas, start_point, end_point)

        # Find the best time in this generation and add it to the list
        best_genome = max(
            population, key=lambda genome: fitness(genome, rutas))
        time = best_genome[1]
...
final_population, best_times = genetic_algorithm(
